using Reqnroll.BoDi;
using nipts_pts_automation_tests.Configuration;
using nipts_pts_automation_tests.HelperMethods;
using OpenQA.Selenium;

namespace nipts_pts_automation_tests.Pages.AP_GB.LandingPage
{
    public class LandingPage : ILandingPage
    {
        private IObjectContainer _objectContainer;

        #region Page Objects

        private IWebElement PageHeading => _driver.WaitForElement(By.XPath("//h1[contains(@class,'govuk-heading-xl')] | //h1[contains(@class,'govuk-fieldset__heading')] | //h1[@class='govuk-label-wrapper'] | //h1[@class='govuk-panel__title'] | //div[@class='govuk-grid-row']//h1[contains(@class,'govuk-heading-l')]"));

        private IWebElement txtLoging => _driver.WaitForElement(By.XPath("//input[@id='password'] | //input[@id='EnteredPassword']"));

        private IWebElement btnContinue => _driver.WaitForElement(By.XPath("//button[contains(text(),'Continue')]"));
        private By AcceptAdditionalCookies => By.XPath("//button[contains(text(),'Accept additional cookies')]");
        private IWebElement HideCookieMessage => _driver.WaitForElement(By.XPath("//div[@id='govuk-cookie-banner-accepted'] //button[contains(text(),'Hide cookie message')]"));

        #endregion Page Objects

        private IWebDriver _driver => _objectContainer.Resolve<IWebDriver>();

        public LandingPage(IObjectContainer container)
        {
            _objectContainer = container;
        }

        #region Page Methods

        public bool IsPageLoaded(string pageName)
        {
            if (_driver.FindElements(AcceptAdditionalCookies).Count > 0)
            {
                _driver.FindElement(AcceptAdditionalCookies).Click();
                HideCookieMessage.Click();
            }
            return _driver.IsHeadingLoaded(pageName);
        }

        public void EnterPassword()
        {
            txtLoging.SendKeys(ConfigSetup.BaseConfiguration.TestConfiguration.EnvPassword);
            btnContinue?.Click();
        }

        public void ClickContinueButton()
        {
            By continueBy = By.XPath("//button[contains(text(),'Continue')]");

            string headingText;
            try
            {
                headingText = PageHeading.Text;
            }
            catch (StaleElementReferenceException)
            {
                // Page has already navigated away from the beta login page; nothing to do.
                return;
            }
            catch (WebDriverException)
            {
                // Heading not present (e.g. already past the landing page); nothing to do.
                return;
            }

            if (headingText.Contains("Private beta testing login"))
            {
                // The password step already clicks Continue, so this page may be mid-navigation: the
                // button can detach between the presence check and the click. A stale/detached button
                // here means the page has already advanced past the beta login, which is the goal.
                try
                {
                    if (_driver.FindElements(continueBy).Count > 0)
                        _driver.FindElement(continueBy).Click();
                }
                catch (StaleElementReferenceException) { }
                catch (WebDriverException) { }
            }
        }

        public bool VerifyNextPageIsLoaded(string pageName)
        {
            // Poll for the target heading rather than reading it once: right after the navigating
            // click the new page may not have rendered yet, so a single read returns the previous
            // page's heading and fails instantly. IsHeadingLoaded waits (textContent fallback,
            // stale-safe) for a //h1|//legend containing pageName.
            return _driver.IsHeadingLoaded(pageName);
        }

        #endregion Page Methods
    }
}