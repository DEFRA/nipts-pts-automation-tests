...
namespace nipts_pts_automation_tests.Pages.CP.Pages
{
    public class WelcomePage : IWelcomePage
    {
        ...
        public void clickOnViewWithSPSUser(string departureRoute)
        {
            ...
            Console.WriteLine($"GBDepartureTime : {GBDepartureTime}");
            Console.WriteLine($"Matching Time: {matchingTime}");
        ...
        {
            bool status = false;
            string NoChecksMsgEle = $"//p[contains(text(),'No animals have been checked in the past 48 hours.')]";
            if (_driver.FindElements(By.XPath(NoChecksMsgEle)).Count > 0)
            {
                return NoChecksMsg.Text.Contains("No animals have been checked in the past 48 hours.");
            }
            else
            {
